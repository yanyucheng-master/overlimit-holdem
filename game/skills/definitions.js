...
function freezeSkill(skill) {
  ...
  const catalogSummary = skill.catalogSummary || skill.shortDescription || skill.description || "";
...
function protocolSkill(id, name, category, catalogSummary, shortDescription, expertExtra) {
  return freezeSkill({
    id,
    name,
    load: 1,
...
const SKILL_DEFINITIONS = Object.freeze({
  DEEP_BREATH: freezeSkill({
    ...
    catalogSummary: "本手不再用技能，结束时回2能量",
    shortDescription: "花 1 点能量缓一口气。如果这手牌你不再用别的技能，结束时会拿回 2 点，相当于净赚 1 点。",
    expertDescription: "主动/秘密。负载 1，费用 1，每手最多 1 次，仅能在自己的合法下注行动回合发动，翻牌前至河牌均可。发动时支付 1 能量；若此后直到本手结束没有再发生任何自己的技能事件，手牌结束时恢复 2 能量。无当前能量上限要求，只受实际能量上限约束。普通 Fold 不取消恢复；若深呼吸后无其他技能事件且自己 Fold：恢复 2，并再获得败局自然 +2。可被反制；费用 1 的失败技能经回收利用计算后实际返还 0。公平会清除待恢复状态并抑制本手全部结束恢复。",
  }),
  RECYCLE: freezeSkill({
    id: "RECYCLE", name: "回收利用", load: 2, energyCost: 0,
    tags: [SKILL_TAGS.PASSIVE, SKILL_TAGS.RESOURCE],
    catalogSummary: "返还最贵失败技能50%能量",
  ...
  INTIMIDATION: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, canBeCountered: true,
    catalogSummary: "禁止弃牌，双方本手投入上限500",
  ...
  DESPERATION: freezeSkill({
    ...
    catalogSummary: "开手筹码≤200，获胜收益×3并回1能量",
  ...
  BLOOD_BATTLE: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, canBeCountered: true,
    catalogSummary: "本手筹码输赢×2，双方发动则×4",
  ...
  DEFENSE: freezeSkill({
    id: "DEFENSE", name: "防守", load: 3, energyCost: 4,
    ...
    catalogSummary: "未弃牌却落败时，最终损失减半",
    shortDescription: "偷偷垫一层。如果你没弃牌却输掉这手，最终亏的筹码会减半。",
    expertDescription: "主动/秘密。负载 3，费用 4，每手最多 1 次，翻牌前至河牌任意自己的合法下注行动回合可发动。若自己最终输掉本手并且不是主动 Fold，最终净筹码损失减半；覆盖整手最终损失，与发动阶段无关。赢、平局或主动 Fold 则无收益，已支付 4 能量不返还。发动本身秘密；若结算时确实把一个公开损失减半，此时可以自然确认「防守」。",
  }),
  PERCEPTION: freezeSkill({
    ...
    visibility: "SECRET",
    catalogSummary: "概率获得真假底牌情报，落后时更易触发",
  ...
  TOP_SECRET: freezeSkill({
    ...
    visibility: "SECRET",
    catalogSummary: "每手默认戒备，可关闭；触发后持续保护底牌",
    shortDescription: "每手默认开启秘密戒备，可主动关闭一次且本手不能重开。开启时若有人侵犯底牌信息且能量足够，自动支付3能量阻止并持续保护本手。",
    expertDescription: "被动/秘密/信息防御。负载3，费用3。每手开始默认ON（ARMED），可在任一方回合主动关闭一次，关闭后本手锁定OFF；操作不消耗能量、不占行动、不产生技能事件。开启时敌方真正读取、推断、交换、零化或操作私人底牌，若能量≥3且允许新技能事件，同步支付3并阻止，随后锁定ON（ACTIVE_LOCKED），持续保护本手且不重复收费。能量不足时效果通过，戒备仍ON，之后可恢复能量再触发，也可主动关闭。阻挡感知底牌事实、情报底牌、千术对手底牌和零化底牌，不影响公共牌、未来牌或灵视等纯技能元信息查询。攻击方正常支付费用，收到通用失败，可按原规则回收；不直接获知绝密身份或戒备状态。成功保护是真实技能事件，会打断深呼吸，但不触发警觉或反制；关闭戒备不打断深呼吸。公平清除保护并禁止本手重启；绝路阻止新启动但不取消既有保护。本人状态重连保留，下一手重置ON。公共手牌审计不披露该防御事件；灵视仍按既有授权查询本手已发生技能。",
  }),
  COUNTER: freezeSkill({
    ...
    catalogSummary: "废掉对手下一主动技能，并封锁其后续技能",
  ...
  FAIRNESS: freezeSkill({
    ...
    catalogSummary: "清除未完成技能状态，并封锁本手技能与回能",
    shortDescription: "当场清掉双方还没结算完的技能状态，这手谁都不能再动技能，结束时也不再回能量。已经换完的牌不会变回去。",
    expertDescription: "主动/公开/全局控制。负载 4，费用 3，每手最多 1 次，可在自己的任意合法下注行动回合使用，不必是本手第一个技能。不能被反制阻止。发动成功后立即清除双方当前存在的持续技能、预埋技能、待结算技能、尚未完成的技能状态（含血战、防守、恐吓、绝密保护、绝境待结算、反制陷阱、深呼吸待恢复、零化、撤退、未触发试探、伪装）；本手剩余时间双方均不能发动主动技能或产生新的被动技能事件；本手所有手牌结束能量恢复全部取消，包括胜者自然 +1、败者自然 +2、平局双方 +1、深呼吸恢复、绝境恢复、资源型强运恢复及其他技能手牌结束恢复。公平不是时间回溯：不能恢复已经交换完成的牌、已经改变完成的牌堆、已经看见的信息、已经发生的筹码变化、已经完成的终局直接筹码转移。零化属于持续结算状态，因此公平可以取消零化。公平成功后本手不能再发动终局。公平仅在违约前免除贷款利息：应还金额变为实际取得的本金。本金仍需真实偿还，贷款债务锁、借款手、两手宽限期和本手额度不重置。重复公平不能继续减债。违约后的本金、利息与一次性违约金都不受公平影响。偿还不是技能事件，公平不禁止主动偿还。",
  }),
  CHEAT: freezeSkill({
    ...
    catalogSummary: "用自己底牌交换对手、公共牌或牌堆牌",
  ...
  DEAD_END: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, canBeCountered: true,
    catalogSummary: "最大全押并封锁对手技能；其弃牌则收益×3",
    shortDescription: "立刻按规则允许的最大 ALL IN，并让对手这手不能再动技能。如果对手弃牌，你这手净赢筹码变成三倍。",
    expertDescription: "主动/公开/ALL IN。负载 5，费用 5，每手最多 1 次，任意合法下注阶段可用。成功发动后自动执行当前规则允许的最大合法 ALL IN，然后对手本手不能再发动主动技能或产生新被动技能事件，但不取消已经完成或已经存在的既定事实。若对手之后 Fold：绝路玩家标准净收益 ×3。若对手 Call 并进入 Showdown，即使绝路玩家获胜，绝路本身不提供 ×3。恐吓状态下仍可发动；ALL IN 实际投入最多达到累计 500，但必须记录为 ALL IN 行为。由于恐吓禁止 Fold，绝路的 Fold ×3 分支在该情况下不会发生。",
  }),
  CLAIRVOYANCE: freezeSkill({
    id: "CLAIRVOYANCE", name: "灵视", load: 3, energyCost: 2,
    tags: [SKILL_TAGS.ACTIVE, SKILL_TAGS.INFORMATION, SKILL_TAGS.SECRET, SKILL_TAGS.ONCE_PER_HAND],
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, visibility: "SECRET", canBeCountered: true,
    catalogSummary: "查看对手真实能量与本手已发生技能",
  ...
  NULLIFICATION: freezeSkill({
    ...
    catalogSummary: "让指定公共牌或随机对手底牌不参与本手算牌",
  ...
  LOAN: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 2, requiresActionTurn: true, visibility: "MIXED", canBeCountered: true,
    catalogSummary: "借筹码或能量，欠款封锁后续贷款",
    shortDescription: "每手最多贷款两次：筹码分支最多两次，能量分支最多一次。债务按笔主动偿还；借款手后欠款封锁新贷款。第二个宽限手结算后仍可按原金额偿还；进入下一手时未还，每笔加一次违约金。",
    expertDescription: "主动/资源。负载 2，费用 2，仅自己的合法下注行动回合发动。每手总共最多 2 次，筹码分支最多 2 次，能量分支最多 1 次。分支在扣费前锁定。筹码贷款公开：立即从对手转移最多 100 筹码，每笔正常应还 150；两次筹码贷款保留 200 筹码斩杀线。能量贷款秘密：扣费后立即获得最多 5 能量，不突破个人上限，每笔正常应还 6。借款当手仍可完成剩余额度；进入后续手时，只要任何一笔未还，便不能新增贷款。可按笔主动完整偿还，不支持部分还款，不会自动扣款或追缴未来收益。偿还不是技能事件，不耗发动费用、不触发反制或警觉、不打断深呼吸、不推进下注行动，也不重置本手次数；公平封锁不禁止偿还。筹码偿还经实际转移给原对手；手中双方均未全下且偿还后仍有筹码时可还，否则等待安全结算窗口。能量偿还必须有足额真实能量，不允许透支。借款手之后有两个完整宽限手。第二个宽限手全部结算完成后，仍可使用最终筹码与能量按原金额偿还；只有进入再下一手前仍未还，每笔筹码债务才加一次 25，能量债务加一次 1。违约只锁定贷款，不影响其他技能或普通行动。公平仅在违约前免除利息：应还金额变为该笔实际取得的本金；不清本金、不解锁、不重置宽限或次数，重复公平不继续减债。违约后公平对该债务完全无效；公平后仍违约则在本金上加一次违约金。全部真实还清后贷款立即恢复，仍受当前手次数限制。反制捕获贷款时费用照付，不获得资源，不生成债务，不改变债务状态。比赛合法结束时未偿债务终止，不做赛后追债。伪装下公开贷款只播报行为，不公开数字。",
  }),
  ALERT: freezeSkill({
    id: "ALERT", name: "警觉", load: 1, energyCost: 0,
    tags: [SKILL_TAGS.PASSIVE, SKILL_TAGS.INFORMATION, SKILL_TAGS.SECRET],
    visibility: "SECRET",
    catalogSummary: "概率察觉秘密行动，连续未中会提高概率",
  ...
  RETREAT: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, visibility: "SECRET", canBeCountered: true,
    catalogSummary: "秘密弃牌，并返还双方本手标准投入",
    shortDescription: "先偷偷准备后路。这手如果你主动弃牌，双方这手丢进锅里的筹码会原路返还，当这手没输没赢。",
    expertDescription: "主动/秘密/防御。负载 2，费用 3，每手最多 1 次，仅能在自己的合法下注行动回合发动。成功建立后，可在当前同一个下注决策窗口立即 Fold，也可保留状态等到本手之后的合法 Fold 窗口使用；不要求提前一街或最小延迟。撤退 Fold 只退还双方本手全部标准下注贡献（含盲注、跟注、加注与标准 ALL IN 贡献），标准净筹码转移为 0，且不获得普通败者 +2，对手正常恢复 1（公平抑制时除外）。不回滚贷款/终局等直接技能筹码转移、已付能量、已见信息或已完成换牌。若被反制：3 能量已付、状态不得建立、不自动 Fold，玩家仍留在原下注决策窗口。绝路不会清除已有撤退，因此先撤退后绝路仍可撤退 Fold，此时标准净收益为 0，绝路 Fold×3 仍为 0。先绝路则不能再新发动撤退。公平清除撤退且不退 3 能量。恐吓禁止 Fold，因此即使状态仍在也无法触发。终局关闭下注后撤退状态可以仍在，但没有合法 Fold 窗口。撤退 Fold 不触发试探。",
  }),
  RESTART: freezeSkill({
    ...
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, visibility: "SECRET", canBeCountered: true,
    catalogSummary: "洗回自己的两张底牌并重新抽2张",
  ...
  DISGUISE: freezeSkill({
    ...
    catalogSummary: "向对手隐藏双方筹码、底池与下注金额",
  ...
  ENDGAME: freezeSkill({
    ...
    tags: [SKILL_TAGS.ACTIVE, SKILL_TAGS.CONTROL, SKILL_TAGS.SETTLEMENT, SKILL_TAGS.ONCE_PER_HAND],
    allowedPhases: ACTIVE_STREETS, maxUsesPerHand: 1, requiresActionTurn: true, visibility: "PUBLIC", canBeCountered: true,
    catalogSummary: "锁注没收对手多余投入并强制摊牌",
