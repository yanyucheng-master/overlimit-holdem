...
def add_cover(doc):
    ...
    meta_rows = [
        ...
        ["冻结项", "感知 FROZEN_V1 / spec-25-50；强运 FROZEN_V1 / soft-v1；贷款 V1.0 主动偿还"],
        ["规则日期", "感知 / 强运 2026-08-20；贷款主动偿还 2026-09-13"],
...
def build():
    ...
    for t in [
        "每手结束：所有能量恢复、Loan 宽限与违约、Fairness 抑制、Fortune 资源效果等全部处理结束后，才刷新公开快照。",
    ...
    for text in [
        ...
        "两手宽限与违约",
        "借款手为 N，不计入宽限。N+1 为第一宽限手，N+2 为第二宽限手；N+2 正式结束仍未偿还才违约。Showdown、普通 Fold、Tie、Retreat 均计一手。",
    ...
    add_callout(doc, "解冻前禁止改：感知触发 / 真假概率、强运 soft-v1 数值、改牌费用 3、能量下限 -4、贷款 100 / 150 与费用 2、公平负载 4 / 费用 3。贷款采用主动偿还、两手宽限和违约前公平去利息的正式规则，产品版本保持 V1.0。")
