...
def add_cover(doc):
    ...
    add_callout(doc, "本文档是与当前代码一致的规则基线，不是重新设计稿。感知与强运保持 FROZEN_V1。贷款采用 V1.0 主动逐笔偿还与两手宽限规则，100 / 150 与公平负载费用不再因此削弱。")

    meta_rows = [
        ["文档性质", "现行实现对照，可直接作为开发 / 审阅基线"],
        ["首发规模", "24 个主体技能 + 9 个协议"],
        ["构筑约束", "最少 1 个、最多 4 个，总负载上限 8"],
        ["冻结项", "感知 FROZEN_V1 / spec-25-50；强运 FROZEN_V1 / soft-v1.1-energy；贷款 V1.0 主动偿还"],
        ["规则日期", "感知 2026-08-20；强运校准 2026-09-15；贷款主动偿还 2026-09-13"],
...
def build():
    ...
    add_body(doc, "公平成功后，本手全部结束恢复都被抑制，包括胜者 +1、败者 +2、平局双方 +1、深呼吸、绝境 +1、反制空放返还、资源型强运。")
    ...
    add_table(
        ...
        [
            ...
            ["18", "贷款", "2", "2", "主动 / 资源", "混合", "合计最多 2；筹码最多 2 / 能量最多 1"],
    ...
    skill(
        ...
        [
            "每手默认开启秘密戒备，可在任一方回合主动关闭一次，随后本手锁定 OFF；关闭不耗能、不占行动、不产生技能事件。",
            "开启时遭遇私人底牌访问，若能量 ≥ 3 且允许新技能事件，同步付 3 并锁定 ON，持续保护本手且不重复收费。能量不足则攻击通过，仍保持戒备；没有反应等待窗口。",
            "阻挡：感知底牌信息、情报底牌、千术对手底牌、零化底牌。",
            "不阻挡：公共牌情报、灵视、纯技能元信息。",
            "攻击方只收到通用失败，付费失败仍可回收；公共审计不自动披露身份，灵视的既有授权查询不受影响。本人状态重连保留，下一手重置 ON。",
            "关闭不触发反制/警觉、不打断深呼吸；真正防御会打断深呼吸。公平清除保护并禁止重启，绝路不追溯清除已有保护。",
        ],
    )
    skill(
        "10  反制  /  Counter",
    ...
    skill(
        ...
        [
            ...
            "包括：反制陷阱、血战、防守、恐吓、绝密当前保护、绝境待结算、深呼吸待恢复、零化、撤退、未触发试探、伪装等。",
            ...
            "贷款不属于可清除的未完成状态；仅违约前可免除利息，保留实际本金。",
            "公平不解锁贷款，不重置次数、借款手或宽限；重复使用不继续减债。",
            "违约后公平对本金、利息和违约金完全无效。偿还不受技能封锁影响。",
        ],
    )
    skill(
        "12  千术  /  Cheat",
    ...
    skill(
        ...
        [
            ...
            "底牌概率 6%～16%、公共牌 4%～10%、资源 12%～20%；筹码/能量权重仍为 0.78/0.22、0.74/0.26、0.40/0.60。",
    ...
    add_skill_meta(doc, "负载 2 · 费用 2 · 主动 · 筹码公开 / 能量秘密")
    add_table(doc, ["分支", "获得", "正常应还", "一次性违约金"], [
        ["筹码", "最多 100", "150", "+25"], ["能量", "最多 5", "6", "+1"],
    ])
    for text in [
        ...
        "债务状态：无债务时可借款；借款当手可完成剩余额度；借款手结束仍欠款时进入宽限，禁止新增贷款；违约后仍只封锁贷款。全部真实偿还后恢复可用。",
        ...
        "Fairness 与贷款",
        "仅违约前免除利息，应还金额设为该笔实际取得的本金：通常 150 → 100、6 → 5。",
    ...
    for t in ["正常赢：自然恢复 +1，计入撤退费用后净 -2。", "正常输：仍有败者 +2，净 -1。"]:
        add_bullet(doc, t)
    add_body(doc, "以上净值仅计撤退费用与自然恢复；其他技能、能量上限与公平抑制仍按各自规则处理。")
    ...
    add_table(
        ...
        [
            ["公平 × 反制", "公平免疫反制"],
            ["公平 × 贷款", "仅违约前免除利息，保留实际本金，不解锁或重置宽限与额度；违约后对债务无效"],
    ...
    add_table(
        ...
        [
            ...
            ["贷款债务", "V1.0 正式规则", "主动逐笔偿还 / 两手宽限 / 一次性违约金", "2026-09-13"],
            ["贷款数值", "V1.0 现行规则", "100 / 150、5 / 6、费用 2、每手总计最多 2", "2026-09-13"],
            ["公平数值", "现行冻结", "负载 4 / 费用 3", "2026-08-21"],
        ],
        col_widths=[3.2, 4.4, 4.4, 4.4],
    )
    add_callout(doc, "解冻前禁止改：感知触发 / 真假概率、强运 soft-v1.1-energy 数值、改牌费用 3、能量下限 -4、贷款 100 / 150 与费用 2、公平负载 4 / 费用 3。贷款采用主动偿还、两手宽限和违约前公平去利息的正式规则，产品版本保持 V1.0。")
