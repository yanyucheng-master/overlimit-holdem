    ...
    },
    {
      ...
      content: `
        ...
        <section id="rule-energy-recovery" data-rule-entry data-rule-entry-title="手牌结束自然恢复">
          <h4>手牌结束自然恢复</h4>
          <div class="rules-table-wrap"><table class="rules-table"><thead><tr><th>手牌结果</th><th>自然恢复</th></tr></thead><tbody><tr><th scope="row">获胜</th><td>+1</td></tr><tr><th scope="row">失败或普通弃牌</th><td>+2</td></tr><tr><th scope="row">平局</th><td>双方各 +1</td></tr><tr><th scope="row">撤退弃牌</th><td>撤退者 +0，对手 +1</td></tr></tbody></table></div>
          <p>技能造成的额外恢复、支付、借贷、债务或恢复抑制另行结算。</p>
        </section>
        <section id="rule-energy-public" data-rule-entry data-rule-entry-title="对手公开能量">
          <h4>对手公开能量</h4>
          <p>对手能量按“逐手公开、手内冻结”显示：一手结束并完成全部资源结算后更新。结束阶段的私有能量偿还不会即时公开；下一手开始前统一更新为最终能量，手内保持不变。普通公开范围为 0～8；真实值低于 0 时显示 0，9 或 10 时显示 8。</p>
        ...
        <div class="rules-table-wrap">
          <table class="rules-table rules-interaction-table">
            ...
            <tbody>
              ...
              <tr id="interaction-fair-loan" data-rule-entry data-rule-entry-title="公平 × 贷款"><th scope="row">公平 × 贷款</th><td>仅在违约前免除利息，保留实际本金。不解锁、不重置宽限或额度。违约后对债务无效。</td></tr>
    ...
    },
    {
      ...
      content: `
        <section id="rule-tie" data-rule-entry data-rule-entry-title="平局">
          <h4>平局</h4>
          <p>摊牌时双方最终最佳 5 张完全相同，则判定为平局。双方不获得牌型基础奖励，不产生普通胜利倍率，双方各自然恢复 1 能量；公平生效时不恢复。</p>
        ...
        <section id="rule-debt-expiry" data-rule-entry data-rule-entry-title="贷款债务终止">
          <h4>贷款债务终止</h4>
          <p>若比赛已经合法结束，全部未偿贷款债务与违约状态终止，不再进行赛后偿还，也不能通过赛后债务逆转已经成立的胜负。</p>
          <p>若比赛尚未结束，贷款只能按笔主动偿还，不自动扣款。手中筹码偿还必须避开全下并保留正筹码；安全结算窗口允许完整偿还至零，随后按正常筹码归零规则结束比赛。</p>
        </section>
      `,
    },
    {
  ...
  skills: Object.freeze([
    {
      ...
      keywords: "恢复 2 败者 +2 公平 反制",
      content: `<p>只能在自己的合法下注回合发动。支付 1 点能量后，若直到本手结束都没有再发生自己的其他技能事件，则恢复 2 点能量。</p><p>普通弃牌不取消待恢复；若同时属于本手败方，仍可另行获得普通败者 +2 能量。可被反制；若作为费用 1 的失败技能进入回收利用结算，向下取整后实际返还 0。公平会清除待恢复状态并抑制该手结束恢复。</p>`,
    },
    {
    ...
    },
    {
      ...
      meta: ["负载 3", "能量 3", "被动 · 秘密", "默认ON · 每手可关闭一次"],
      keywords: "底牌保护 读取 推断 交换 零化 情报 千术 感知",
      content: `<p>被动/秘密/信息防御。负载3，费用3。每手开始默认ON（ARMED），可在任一方回合主动关闭一次，关闭后本手锁定OFF；操作不消耗能量、不占行动、不产生技能事件。开启时敌方真正读取、推断、交换、零化或操作私人底牌，若能量≥3且允许新技能事件，同步支付3并阻止，随后锁定ON（ACTIVE_LOCKED），持续保护本手且不重复收费。能量不足时效果通过，戒备仍ON，之后可恢复能量再触发，也可主动关闭。阻挡感知底牌事实、情报底牌、千术对手底牌和零化底牌，不影响公共牌、未来牌或灵视等纯技能元信息查询。攻击方正常支付费用，收到通用失败，可按原规则回收；不直接获知绝密身份或戒备状态。成功保护是真实技能事件，会打断深呼吸，但不触发警觉或反制；关闭戒备不打断深呼吸。公平清除保护并禁止本手重启；绝路阻止新启动但不取消既有保护。本人状态重连保留，下一手重置ON。公共手牌审计不披露该防御事件；灵视仍按既有授权查询本手已发生技能。</p>`,
    },
    {
    ...
    },
    {
      ...
      keywords: "公平 去利息 本金 违约 自然恢复",
      content: `<p>主动/公开/全局控制。负载 4，费用 3，每手最多 1 次，可在自己的任意合法下注行动回合使用，不必是本手第一个技能。不能被反制阻止。发动成功后立即清除双方当前存在的持续技能、预埋技能、待结算技能、尚未完成的技能状态（含血战、防守、恐吓、绝密保护、绝境待结算、反制陷阱、深呼吸待恢复、零化、撤退、未触发试探、伪装）；本手剩余时间双方均不能发动主动技能或产生新的被动技能事件；本手所有手牌结束能量恢复全部取消，包括胜者自然 +1、败者自然 +2、平局双方 +1、深呼吸恢复、绝境恢复、资源型强运恢复及其他技能手牌结束恢复。公平不是时间回溯：不能恢复已经交换完成的牌、已经改变完成的牌堆、已经看见的信息、已经发生的筹码变化、已经完成的终局直接筹码转移。零化属于持续结算状态，因此公平可以取消零化。公平成功后本手不能再发动终局。公平仅在违约前免除贷款利息：应还金额变为实际取得的本金。本金仍需真实偿还，贷款债务锁、借款手、两手宽限期和本手额度不重置。重复公平不能继续减债。违约后的本金、利息与一次性违约金都不受公平影响。偿还不是技能事件，公平不禁止主动偿还。</p>`,
    },
    {
    ...
    },
    {
      ...
      meta: ["负载 5", "能量 5", "主动 · 公开", "每手 1 次 · 本人下注回合"],
    ...
    },
    {
      ...
      content: `<p>在底牌、翻牌、转牌、河牌与手牌结束资源阶段自动判定。判定概率受自身筹码劣势与当前能量影响：底牌 6%～16%、公共牌 4%～10%、资源 12%～20%。牌面改良成功时支付 3 点能量；资源型强运成功时额外获得 1 点能量，不支付改牌费用，也不会递归触发强运。</p><p>强底牌不会被强制改动；较差时可自动尝试改善。玩家不能选择是否触发、换哪张或换成哪张。公共牌判定不得读取对手底牌。强运允许真实能量最低降至 -4；若继续支付会低于 -4，则该次改牌不发生。负能量期间只有强运可以产生新的技能事件。</p>`,
    },
    {
    ...
    },
    {
      ...
      content: `<p>在自己的合法下注窗口建立撤退状态。可在同一行动窗口立即弃牌，也可在之后的其他合法弃牌窗口使用。</p><p>撤退弃牌时，该手立即结束，双方本手全部标准下注贡献原路退还，标准净筹码转移为 0，使用者不获得普通败者 +2 能量，对手正常恢复 1（公平抑制时除外）。不会回滚贷款或终局直接转移、已支付能量、已获信息、换牌或牌堆修改。</p><p>状态未用于弃牌时按正常结果结算。被反制时费用已付、状态不建立，玩家留在原行动窗口。公平可清除状态；恐吓下不能完成弃牌；撤退弃牌不触发试探。</p>`,
    },
    {
